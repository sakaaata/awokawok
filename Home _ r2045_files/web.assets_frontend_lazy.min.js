...
if(_.has(odoo.__DEBUG__.services,'website.root')){odoo.define('saas_website.db_activation_website',function(require){'use strict';var ExpirationTag=require('saas_trial.db_expiration_tag');if(!$('.o_menu_systray').length){return;}
if($('div.o_menu_systray>div').length>0){ExpirationTag.include({template:'saas_trial.db_expiration_tag_website_15'});}
var WebsiteRoot=require('website.root').WebsiteRoot;WebsiteRoot.include({xmlDependencies:(WebsiteRoot.prototype.xmlDependencies||[]).concat(['/saas_trial/static/xml/trial.xml']),start:function(){return this._super.apply(this,arguments).then(function(){var tag=new ExpirationTag(this);tag.prependTo($('.o_menu_systray'));});},});});}});;
